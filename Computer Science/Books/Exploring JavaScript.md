...
This operator is especially useful for **optional values**, when a returned value can either be `null` or an actual value.
An example of this would be trying to find a match to a [[Regular Expression]]. If it can't find anything, `null` will be returned instead of an actual match.
Another example would be when trying to return a property of an object. If it doesn't exist, give a default value instead.
## 17 - Booleans
A value is truthy or falsy **when it is `true` or `false` when converted to a boolean.**
Here's all possible falsy values, everything else (including objects) is `true`:
- `undefined`
- `null`
- Boolean: `false`
- Numbers: `0`, `NaN`
...
- All regular numbers (floating points and integers) are **64-bit doubles**
...
Some literals allow you to express integers in different bases, like `0b`, `0o` and `0x` to represent binary, octal and hex respectively.
There's also literals for scientific notation of floating points.
#### 18.2.3 - Syntactic pitfall
Careful using a method on an integer directly, as it might result in a `SyntaxError`:
```js
7.toString(); // SyntaxError
```
There are four ways to remedy to that:
- (7).toString(2)
- 7.0.toString(2)
- 7..toString(2)
- 7 .toString(2) // space before dot
Non-decimal integer literals (when you use a specific base) don't have this pitfall.
#### 18.2.4 - Underscores as separators
You can use underscores as separators in number literals to make reading long numbers easier.
That works with other bases too, such as binary, but grouping is important (e.g. byte by byte).
**It will probably block parsing functions like `Number()` and `Number.parseInt()` from working, though.**
### 18.5 - `NaN` and `Infinity`
These are the two JavaScript error values:
- `NaN` is returned if parsing a number doesn't work or an operation can't be performed.
  You can check for `NaN` with `Number.isNaN()`. **NaN is not strictly equal (`===`) to itself.**
- `Infinity` is returned if a number is too large or if a division by zero occurred.
  You can check for `Infinity` with `Number.isFinite()` or by comparing with `===`.
## 19 - `Math`
`Math` is an object with data properties and methods for processing numbers.
## 20 - Bigints
BigInts are integers that grow and shrink in storage space as their contents needs.
#### 20.9.1 - When to use BigInts vs regular numbers
- Use numbers for up to 53 bits, and for Array indices.
- BigInts should be used for large numeric values. If your values don't fit into 53 bits, you don't have a choice anyway.
## 21 - Unicode
