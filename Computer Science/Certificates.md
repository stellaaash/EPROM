...
It uses asymmetric cryptography ([[Public-key Cryptography]]) to produce a signature using a private key, which can then be verified using a public key.
...
# Chain of Trust
Certificates have a so-called *chain of trust*, starting with a root CA (*certificate authority*). Those are often trusted by devices in factory settings, and web browsers do the same.
